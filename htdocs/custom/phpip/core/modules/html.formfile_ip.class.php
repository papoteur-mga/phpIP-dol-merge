...
/**
 *	\file       htdocs/core/class/html.formfile_ip.class.php
 *  \ingroup    core
...
class FormFile
{
     ...
     * 		@param		string				$company		the name of the company to list its matters
     * 		@param		string				$codelang			Default language code to use on lang combo box if multilang is enabled
     * 		@return		int										<0 if KO, number of shown files if OK
     */
    function show_documents($modulepart,$modulesubdir,$filedir,$urlsource,$genallowed,$delallowed=0,$modelselected='',$allowgenifempty=1,$forcenomultilang=0,$iconPDF=0,$maxfilenamelength=28,$noform=0,$param='',$title='',$buttonlabel='',$company='',$codelang='')
    {
        $this->numoffiles=0;
        print $this->showdocuments($modulepart,$modulesubdir,$filedir,$urlsource,$genallowed,$delallowed,$modelselected,$allowgenifempty,$forcenomultilang,$iconPDF,$maxfilenamelength,$noform,$param,$title,$buttonlabel,$company,$codelang);
     ...
     */
    function showdocuments($modulepart,$modulesubdir,$filedir,$urlsource,$genallowed,$delallowed=0,$modelselected='',$allowgenifempty=1,$forcenomultilang=0,$iconPDF=0,$maxfilenamelength=28,$noform=0,$param='',$title='',$buttonlabel='',$company='',$codelang='',$morepicto='')
    {
        ...
        if ($genallowed)
        {
            ...
            if ($modulepart == 'company')
            {
                ...
                else
                {
                    ...
                    include_once DOL_DOCUMENT_ROOT.'/custom/phpip/core/modules/modules_ipfiles.class.php';
                    $filelist=ModeleThirdPartyMatter::liste_matter($this->db, $company);
                }
            }
            ...
            {
                $out.= '<th align="center" class="formdoc liste_titre maxwidthonsmartphone">';
                $out.= '<span class="hideonsmartphone">'.$langs->trans('Matter').' </span>';
                if (is_array($filelist) && count($filelist) == 1)    // If there is only one element
                {
                    $arraykeys=array_keys($filelist);
                    $fileselected=$arraykeys[0];
                }
                $out.= $form->selectarray('matter', $filelist, $fileselected, $showempty, 0, 0, '', 0, 0, 0, '', '');
                $out.= '</th>';
            }
            else
            {
                $out.= '<th align="left" class="formdoc liste_titre">';
                $out.= $langs->trans("Matter");
                $out.= '</th>';
            }
